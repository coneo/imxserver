...
__ENTER_FUNCTION_FOXNET
	...
	std::cout<<"SocketInputStream destructure...\n";
	...
	if ( m_Head <= m_Tail ) 
	{
		if ( m_Head == 0 ) 
		{
			...
			if( nFree != 0 )
			{
				...
				g_pLog->log_info("receive1:%u",nReceived);
			...
			if( nReceived == nFree ) 
			{
				...
				if ( available > 0 ) 
				{
					...
					g_pLog->log_info("receive2:%u",nReceived);
					if (nReceived==SOCKET_ERROR_WOULDBLOCK) return 0 ; 
		...
		else 
		{
			...
			nReceived = m_pSocket->receive( &m_Buffer[m_Tail], nFree );
			g_pLog->log_info("receive3:%u",nReceived);
			if( nReceived==SOCKET_ERROR_WOULDBLOCK ) return 0 ; 
...
//				Assert( m_Tail == 0 );
				...
				if( nFree!=0 )
				{
					...
					g_pLog->log_info("receive4:%u",nReceived);
					if( nReceived==SOCKET_ERROR_WOULDBLOCK ) return 0 ; 
				...
				if( nReceived==nFree ) 
				{
					...
					if ( available > 0 ) 
					{
						...
						nReceived = m_pSocket->receive( &m_Buffer[m_Tail] , available );
						g_pLog->log_info("receive5:%u",nReceived);
						if (nReceived==SOCKET_ERROR_WOULDBLOCK) return 0 ; 
	...
	{	
		...
		{
			nReceived = m_pSocket->receive( &m_Buffer[m_Tail], nFree ) ;
			g_pLog->log_info("receive6:%u",nReceived);
			if( nReceived==SOCKET_ERROR_WOULDBLOCK ) return 0 ; 
		...
		if( nReceived==nFree ) 
		{
			...
			if ( available>0 ) 
			{
				...
				nReceived = m_pSocket->receive( &m_Buffer[m_Tail], available ) ;
				g_pLog->log_info("receive7:%u",nReceived);
				if( nReceived==SOCKET_ERROR_WOULDBLOCK ) return 0 ; 
