...
bool Player::ProcessInput( )
{
	...
	{
		g_pLog->log_error("%u,%u,ProcessInput...m_pSocketInputStream->Fill error",PlayerID(),0);
		return false;
	}
...
bool Player::SendPacket(Packet* pPacket)
{
	...
	if(m_pSocketOutputStream!=NULL)
	{
		...
		g_pLog->log_debug("%u,%u,Scene SendPacket() .port:%u,packID:%u,packSize:%u",PlayerID(),0,m_pSocketOutputStream->m_pSocket->m_Port,pPacket->GetPacketID(),pPacket->GetPacketSize());
